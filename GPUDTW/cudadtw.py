...
def cuda_dtw_metric(X_nld, Y_cld, params) -> MetricResponse:
    ...
    func = cuda_dtw_prepare()
    GPU_Param = get_CUDA_Param()
    if GPU_Param["Max_Threads_pre_Block"] > X_nld.shape[1]:
        raise ValueError("Time series length does not fit GPU")

    # fold the most inner dimension
    X_nl = X_nld.squeeze(-1).astype(numpy.float32)
    Y_cl = Y_cld.squeeze(-1).astype(numpy.float32)
